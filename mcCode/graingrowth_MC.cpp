...
// Current Version Maintained by Sagar Bhatt (bhatts8@rpi.edu)
...
double lambda = 5.0e-3;
double L_initial = 40.0e-3;
double L0 = 40.0e-3;
double K1 = 0.01; //Take a look at this  number as well
double n1 = 0.55;
double m = 1.0 / n1;
double Q = 312000;
double n = 2;
...
int nMCS = 1000;
std::vector<double> tmpDir1;
std::vector<std::vector<double> > tempVector;
std::vector<std::vector<std::vector<double> > > transientTempVector;

void getTempFromFile()
{
  std::ifstream fin("../tempStructuredGridMCS.txt");
  for (int k = 0; k < nMCS; ++k)
  {
    double tmpTempVariable;
    for (int i = 0; i < dim_x; ++i)
    {
      ...
      for (int j = 0; j < dim_y; ++j)
      {
        ...
        {
          fin >> tmpTempVariable;
          tmpDir1.push_back(tmpTempVariable);
        }
      }
      tempVector.push_back(tmpDir1);
      tmpDir1.clear();
    }

    transientTempVector.push_back(tempVector);
    tempVector.clear();
  }
...
template <int dim> void UpdateLocalTmp( int currentIteration, MMSP::grid<dim, unsigned long>& grid, long double physical_time)
{
  ...
  if (dim == 2) {
    for (int codx = x0(grid, 0); codx < x1(grid, 0); codx++)
      for (int cody = x0(grid, 1); cody < x1(grid, 1); cody++) {
        coords[0] = codx;
        coords[1] = cody;
        grid.AccessToTmp(coords) = 273.0+ transientTempVector[currentIteration][coords[0]][coords[1]];
      }
  }
  else if (dim == 3) {
    for (int codx = x0(grid, 0); codx < x1(grid, 0); codx++)
...
unsigned long generate( MMSP::grid<dim, unsigned long >*& grid, int seeds, int nthreads)
{
  ...
  if (dim == 2) {
    ...
    if (number_of_fields == 0) number_of_fields = static_cast<int>(float(dim_x * dim_y) / (M_PI * 1 * 1)); /* average grain is a disk of radius XXX
...
template <int dim> unsigned long update(MMSP::grid<dim, unsigned long>& grid, int steps, int steps_finished, int nthreads, int step_to_nonuniform, int iterationsFinished, long double & physical_time, double * temp)
{
...
//      ghostswap(grid); // once looped over a "color", ghostswap.
    ...
    MPI::COMM_WORLD.Barrier();
    UpdateLocalTmp( step + iterationsFinished, grid, physical_time);
    MPI::COMM_WORLD.Barrier();
    ...
    double grain_size1 =  L0 + lambda * (5.0 * (dim_y / total_number_of_grains));
    ...
    double grain_size2 = L0 + lambda * (5.0 * (dim_y / total_number_of_grains));

    // if (rank == 0)
    //   std::cout << "  " << tmc_at_PdenominatorMax_global << "  " << physical_time << " " << grain_size1 << "  " << grain_size2 << std::endl;
