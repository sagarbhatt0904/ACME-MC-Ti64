...
//----------Ti6Al4V Alloy below beta transus

double lambda = 9.0e-5; 
double L_initial = 7.2e-4; 
double L0 = 7.2e-4; 
double K1 = 1; //Take a look at this  number as well
double n1 = 1;
double m = 1.0/n1;
double Q = 184964.37;
double n = 3.4;
   ...
   if(dim==2){
       ...
       for(int codx=x0(grid, 0); codx < x1(grid, 0); codx++) 
         for(int cody=x0(grid, 1); cody < x1(grid, 1); cody++){
           coords[0] = codx;
           coords[1] = cody;
          grid.AccessToTmp(coords) = 1361.0;
// if(codx<=0.5*dim_x)
//   grid.AccessToTmp(coords) = 200; 
// else
//   grid.AccessToTmp(coords) = 2000; 
...
if(rank==0)
std::cout<< "  "<<tmc_at_PdenominatorMax_global<< "  "<<grain_size1<<"  "<<grain_size2<< std::endl;
		update_timer += rdtsc()-start;
	}//loop over step
